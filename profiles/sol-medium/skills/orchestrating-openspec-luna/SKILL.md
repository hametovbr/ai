...
Keep product planning and architecture outside this skill. Resolve implementation details from approved artifacts and code. If requirements/design are missing or contradictory, block the affected unit and return that decision to planning; continue independent authorized work. Do not silently replace Luna with primary-agent implementation or a different model. Treat outcomes, contracts, invariants and project gates as binding; allow an equivalent cheaper local implementation when only the method was suggested, without changing design or acceptance. Existing execution authorization covers routine work; do not add per-task approvals.
...
6. **Handle failures.** Supply missing context or concrete review findings to the worker. After two corrections to an unchanged brief, diagnose before retrying: narrow/revise the unit or use a fresh Luna worker with a reason. Before any expensive retry classify product, test/harness, infrastructure, access or contract failure; record a corrective change or diagnostic reason for the next run. A retry budget alone justifies no repeat. Preserve attempt counts and lineage. Block if no justified next attempt exists. Design changes return to planning and invalidate affected briefs/results.
...
`READY_FOR_REVIEW` is a worker report, not completion. Record each check's command, cwd, candidate identity, exit status, outcome and log. Missing dependencies, skipped checks and infrastructure failure are `BLOCKED/NOT_RUN`, not PASS. H1 evidence does not verify changed H2; rerun affected checks and required final gates. Assess code, dependencies, build, configuration and environment; broaden checking if impact is unclear. Preserve unrelated evidence with a short explicit reason.

Choose the cheapest reliable existing check for each behavior/risk. Development, boundary and whole-change checks are levels to select, not mandatory separate runs per task. Preserve connected acceptance and required project gates; justify extra expensive checks or new test infrastructure by an uncovered risk. Carry rationale and rerun triggers in plan-linked briefs.

Use behavioral tests for material changes and credible failure paths; reproduce bug fixes when feasible. Follow repository policy, including required failing baselines; otherwise no failing baseline is mandatory for every feature. An unreached or non-discriminating assertion proves nothing: correct it and exercise affected behavior. Judge risk by behavioral/security/data effects, including operational documentation, not diff size or file extension. Use proportional inspection/build/validation for reversible low-impact edits. Never weaken assertions, remove tests or waive required behavior to finish. A reviewer needs requirements and actual diff/code, not just the author's report; reviewer agreement is not proof.

Use existing reports plus concise version/check/result/limits records; add artifacts only for acceptance or recovery needs. Preserve original failed attempts when measuring reliability; successful repairs establish functional acceptance, not a rewritten success rate.
