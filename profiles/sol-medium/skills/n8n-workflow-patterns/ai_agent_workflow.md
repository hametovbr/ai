...
> **For agent design depth, use the `n8n-agents-official` skill.** This file covers where an AI agent sits in a workflow's architecture (trigger → agent → output, the `ai_*` sub-node connection types). The `n8n-agents-official` skill owns the design rules: tool selection and `$fromAI` parameters, the system-prompt vs tool-description split, structured output with autoFix, memory and sessionId, human-in-the-loop review, RAG, and chat shell/core/sub-agent topologies. Start there when building or debugging an agent.
...
**Wiring direction**: a sub-node connects FROM itself TO the agent, and the connection lives on the sub-node keyed by its `ai_*` type. With `update_workflow` you add each with an `addConnection` op using `sourceOutput: "ai_tool"` (or `"ai_languageModel"`, etc.). Multiple tools all stack on the same `ai_tool` index 0.
...
For the deep slot mechanics — tool types, `$fromAI` parameters, memory configuration, parser schemas — see **n8n-agents-official**.
...
For the *content* of these (tool descriptions, system prompts, schema design), see **n8n-agents-official** `references/TOOLS.md`, `references/SYSTEM_PROMPT.md`, and `references/STRUCTURED_OUTPUT.md`.

---

## What the deep design lives in n8n-agents-official

This file is the workflow-architecture view. The design depth below is owned by **n8n-agents-official** — go there, don't duplicate it here:

- **Tool configuration** (the four tool types, native vs `.toolWorkflow` vs HTTP Request Tool vs MCP Client, `$fromAI()` anatomy, tool names/descriptions as prompt) → **n8n-agents-official** `TOOLS.md`, and `SUBWORKFLOW_AS_TOOL.md` for wiring a sub-workflow as a tool.
- **Memory configuration** (buffer/window/postgres/redis, `contextWindowLength`, sessionId handling per trigger) → **n8n-agents-official** `MEMORY.md`.
- **Agent vs chain vs classifier choice, prompt engineering, system-prompt vs tool-description split** → **n8n-agents-official** `SYSTEM_PROMPT.md` (and the SKILL.md "Pick the right node" table).
- **RAG chains, structured output, streaming, fallback models** → **n8n-agents-official** `RAG.md` and `STRUCTURED_OUTPUT.md`.
- **Human review / gating destructive tools** → **n8n-agents-official** `HUMAN_REVIEW.md`.
- **Error handling** (tool failures, LLM API errors, retries, error workflows) → **n8n-error-handling-official**, plus the agent-specific notes in **n8n-agents-official**.
- **Performance, security, testing, common gotchas** → **n8n-agents-official** (anti-patterns table and quick-reference checklist) for the agent-specific ones; the workflow lifecycle (test → validate → activate) is in `n8n-workflow-lifecycle-official`.

One workflow-architecture safety note worth restating here: **any tool that fetches third-party content** (HTTP Request, web search, MCP Client, scrapers) can return attacker-controlled text that reaches the agent's context — indirect prompt injection. If the agent can both *read the internet* AND *take an action the user can't undo*, put a guardrail (human review, read-only scopes) between them. The detail lives in **n8n-agents-official** `HUMAN_REVIEW.md` and the **n8n-agents-official** anti-patterns.

---

## Checklist for AI Agent Workflows

Architecture-level checks (the design-level checklist lives in **n8n-agents-official**):
...
- [ ] Tested and activated per the lifecycle (see `n8n-workflow-lifecycle-official`)
...
5. For all design depth — tools, memory, prompts, structured output, RAG, human review, chat topologies — go to **n8n-agents-official**.

**Pattern**: Trigger → AI Agent (Model + Tools + Memory + optional Parser) → Output

**Related**:
- **n8n-agents-official** — the deep agent design guide (tools, memory, prompts, structured output, RAG, human review, chat topologies)
...
- `n8n-workflow-lifecycle-official` — test, validate, and publish the workflow
- **n8n-error-handling-official** — tool-failure and LLM-error handling
