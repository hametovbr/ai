<!-- Preserved long-form guide from the source skill. Read only when routed here by SKILL.md. -->
...
Load the smallest set that covers the current decisions. A webhook flow commonly needs node configuration, expressions, error handling, and lifecycle guidance, but do not load unrelated domains.
...
| "I'll just run `test_workflow` to see what happens" | Invoke `n8n-workflow-lifecycle-official` `references/TESTING.md`. `test_workflow` mocks the trigger only. Slack sends, DB writes, payments all fire for real. Ensure those side effects are already authorized; otherwise ask before running. |
...
| `test_workflow` | Run with the pin data you supply. **Auto-pins triggers, credentialed nodes, and HTTP Request.** Code, Edit Fields, If, Data Tables, Execute Command, file ops, and sub-workflow calls run for real. Run only when any real side effects are already authorized; otherwise ask first. Pin data is per-execution only with no visual indicator in the execution viewer, so tell the user which nodes were pinned after the call. See `n8n-workflow-lifecycle-official` `references/TESTING.md`. |
...
A review agent should call `get_workflow_details` first, walk the checklist top to bottom, and report findings grouped by severity. Fix MUST FIX items when that mutation is already authorized; otherwise present them for user approval before changing the workflow.
