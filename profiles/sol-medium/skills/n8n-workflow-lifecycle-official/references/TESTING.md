...
**Before `test_workflow`, ensure every real effect from a not-auto-pinned node is within existing user authorization. If any effect is not authorized, ask before the call.** A short message when authorization is needed:
...
1. **Check authorization first** (the protocol above). If the planned effects are already authorized, proceed; otherwise ask the user to approve the specific test effects.
