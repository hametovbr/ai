...
1. Test the success path with `test_workflow`. Confirm shape and code. **API workflows almost always have side-effecting downstreams (DB writes, third-party calls, comms), so run the test only when those effects are already authorized; otherwise ask first.** See `n8n-workflow-lifecycle-official` `references/TESTING.md`.
