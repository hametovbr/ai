...
1. **Require user authorization before building.** This creates a workflow on their instance, with credentials. Existing authorization for this build is sufficient; do not ask again.
...
6. If adding the tool to the agent context is already authorized, add it; otherwise ask the user first
...
|---|---|---|
| Building an MCP-extension workflow without user authorization | Surprise creation of workflows on their instance with credentials | Require authorization first; do not repeat a confirmation already given |
