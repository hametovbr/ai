...
These SQL and MongoDB examples come from the source skill. The parent `SKILL.md` is authoritative. Treat every “always,” “never,” “good,” “bad,” and zero-downtime label below as teaching shorthand rather than a requirement. Verify database dialect and version, actual query/write workload, constraints, deployment topology, and recovery policy before applying an example. When an example conflicts with current engine documentation or observed workload, follow the verified evidence.
...
```sql
-- Example for engines where DECIMAL provides the required exact semantics
price DECIMAL(10, 2)  -- $99,999,999.99

-- Approximate floating point is unsuitable when exact monetary equality is required
...
-- Common policy: store an unambiguous instant; verify engine timezone semantics
...
| Common index candidate | Reason to evaluate |
|--------------|--------|
...
-- Candidate for this index shape (customer_id first); verify with EXPLAIN
SELECT * FROM orders WHERE customer_id = 123;
SELECT * FROM orders WHERE customer_id = 123 AND status = 'pending';

-- Usually cannot use this composite index's leading key for a status-only lookup;
-- target-engine skip-scan or other behavior may differ, so verify with EXPLAIN
SELECT * FROM orders WHERE status = 'pending';
```

**Heuristic:** Match leading columns to actual equality/range/order predicates and verify with the target engine's plan; selectivity alone does not determine order.
...
| Missing useful FK index | Slow joins or parent updates/deletes | Add only when query/constraint behavior and write cost justify it |
...
| Practice | WHY |
|----------|-----|
| Defined recovery path | Use rollback or a tested forward fix according to deployment and data-safety constraints |
