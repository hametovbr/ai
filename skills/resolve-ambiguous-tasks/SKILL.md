...
- **Standard:** incomplete context or multiple systems; compact ledgers plus one audit.
...
4. Read [references/review-contracts.md](references/review-contracts.md). Give a fresh subagent only the artifact and its contract—never the author’s conclusion. If available, explicitly invoke `grill-me`; otherwise use `grilling` with the supplied audit prompt. Reconcile findings, then audit claim-to-source coverage.
5. For `research-only`, deliver the audited synthesis, confidence, limitations, and next evidence; stop here.
6. For implementation, require `Readiness: READY`, create a high-level specification, and obtain user approval. **REQUIRED SUB-SKILL for software architecture:** use `making-architecture-decisions`. Run a fresh specification audit, resolve material findings, and obtain approval for the updated specification.
7. **REQUIRED SUB-SKILL:** use `superpowers:writing-plans`; obtain plan approval. Execute with the applicable domain skills, `superpowers:test-driven-development`, and `superpowers:subagent-driven-development`. Finish with `superpowers:verification-before-completion`.
