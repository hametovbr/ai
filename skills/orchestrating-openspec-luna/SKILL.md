...
description: Use when executing an already planned OpenSpec change with Luna Max subagents under a primary agent's control, including execution decomposition, parallel dispatch, verification, integration, and recovery. Not for creating proposals or making architecture decisions.
---

# OpenSpec execution with Luna Max

## Boundary and roles

Turn a ready OpenSpec plan into bounded, verifiable assignments. Optimize for accepted, integrated behavior and low rework, not maximum agent count.

- **Primary agent:** read the plan and code, decompose execution, resolve implementation questions within the approved design, dispatch, inspect evidence and diffs, run checks, integrate clean results, maintain progress, and accept or reject work.
- **Luna Max workers:** implement code, tests, configuration, generated outputs, and fixes. They do not redesign, expand scope, mark OpenSpec tasks complete, or spawn children.
- **Optional fresh Luna Max reviewer:** inspect a fixed candidate read-only; return evidence-backed findings. The primary agent remains the acceptance authority and performs the final integrated review.

Product planning and architecture precede this skill. Do not silently replace missing decisions with worker guesses or write the implementation yourself when a worker stalls. A newly discovered planning gap blocks only affected work: return that gap to the primary agent's planning process outside this skill. Continue independent authorized work. Obtain user input only for an unresolved decision that needs it; do not add per-task approvals.

Read [handoffs.md](references/handoffs.md) before the first dispatch; use its brief, report, and recovery fields without creating a second copy of the specification.

## Default execution strategy

Use a primary-controlled dependency graph with small integration increments. Treat this as an engineering default, not a claim that one scheduling policy is optimal for every repository.

| Observed work shape | Execution choice |
| --- | --- |
| One small coherent change | One Luna worker; primary review; no extra reviewer by default |
| Coupled changes or an unsettled internal contract | Inspect first, then serialize producer and consumer |
| Independent behaviors with isolated resources | A small parallel ready set, normally starting with two workers |
| Many identical mechanical edits | One bounded batch with a complete target list |
| High-risk or broad behavior | Smaller acceptance units and a fresh read-only review before primary acceptance |
| Integration conflict | One Luna integration/fix unit after writers stop |

Prioritize work that unlocks dependent tasks and exposes integration uncertainty early. Prepare detailed briefs for the next ready units, not every downstream unit before interfaces settle. Refill free capacity as prerequisites are accepted instead of waiting for unrelated work to finish. Integrate and check accepted increments throughout execution; do not postpone all integration to the end.

Keep primary-agent attention on contracts, diffs, failures, and acceptance. Delegate bounded code discovery to Luna only when it has a concrete question and output; independently verify findings that determine contracts or safety. Avoid standing teams of planner, critic, tester, and reviewer for every edit. Use extra roles only to address an identified risk. If workers outrun review or repeated collisions/rework appear, reduce concurrency or enlarge a tightly coupled unit rather than adding agents.
...
Preserve the repository's source-of-truth rules. A task brief cannot override a requirement. If artifacts contradict each other, report the concrete conflict and affected task; resolve it before assigning that behavior. Distinguish binding outcomes, contracts, invariants and project gates from suggested local methods. Workers may choose an equivalent cheaper method within the brief; record the reason without reopening settled design. Task-size refinement is in scope; new API semantics, ownership boundaries, migration strategy, or acceptance criteria are planning changes.
...
Prefer a small vertical behavior with its tests. A handful of files is a useful starting point, not a hard cap. Judge risk by effects on behavior, authority, data and recovery, including operational documentation. A work batch, acceptance milestone and chat session need not have the same boundary. Split when the worker must hold unrelated domains, make several independent decisions, or cannot verify the result locally. Keep tightly coupled code together; batch repeated mechanical edits under one brief. Do not split production code and its necessary tests merely to use more agents.
...
Choose the least expensive check that reliably covers each required behavior or material risk, using existing tests/tools first. Development, boundary/integration and whole-change acceptance are available levels, not three mandatory runs per task. Check uncertain integration assumptions early and retain connected acceptance of the delivered scope. Justify any extra expensive run or new test infrastructure by an uncovered risk; project-mandated gates remain binding. Put the check rationale and rerun triggers in the plan-linked brief so workers can find them without conversation history.

For material behavior changes, require tests that exercise the required behavior and credible failure paths; for bug fixes, reproduce the defect when feasible. Follow repository test policy, including required TDD/failing baselines. Otherwise a deliberately failing baseline is not mandatory for every new feature. An assertion that was never reached or cannot distinguish the required behavior supplies no acceptance evidence; correct it and exercise the affected behavior. For reversible low-impact edits, use proportional inspection/build/validation instead of invented tests that mirror implementation. Never accept weakened assertions, skipped gates, changed expected values, or removed tests merely to make checks green.
...
Any change after a check invalidates affected evidence. Assess changes to code, dependencies, build, configuration and environment; rerun affected boundaries and broaden checks when impact is unclear. A short rationale normally suffices. Identify affected checks from the changed dependencies; rerun required final gates on the final integrated snapshot. Never carry PASS from H1 to modified H2 because the fix was small or time is short. If unrelated evidence remains reusable, record the reason.
...
- Before any expensive retry, classify the failure as product, test/harness, infrastructure, access, or an unresolved contract. Record the corrective change or diagnostic reason that makes the next run informative. A retry allowance alone is not justification; stop unchanged known-failing runs and continue independent authorized work.
...
Maintain one change-scoped execution ledger and task briefs/reports in the repository's established location. If none exists, use a local execution directory outside formal OpenSpec artifacts; record its absolute path. Do not introduce a new OpenSpec schema or add orchestration files to a change's public deliverables by default. Use existing reports plus a concise version/check/result/limits record; add artifacts only for acceptance or recovery needs. Retain enough evidence for resumption; do not delete it before handoff. When measuring success rates, preserve original failed attempts separately from successful repairs; functional acceptance does not rewrite reliability history.
