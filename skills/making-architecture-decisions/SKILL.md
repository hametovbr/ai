...
description: Use whenever a task proposes, selects, reviews, simplifies, refactors, or revises architecture, or designs a complex technical solution; including implementation plans that choose components, abstractions, state, services, integrations, migrations, deployment, automation, databases, frameworks, agents, research harnesses, or system boundaries. Invoke before recommending or implementing the design, even when architecture is implicit or unnamed.
---
...
**Invoke this skill before making any architecture choice or designing any complex technical solution.** This is a gating skill, not an optional review after implementation.

Architecture is present even when the task calls the work an implementation plan, automation, agent workflow, research or evaluation harness, test infrastructure, reliability hardening, integration, refactor, or “just tooling.” Invoke the skill at the first point where the work could introduce or choose components, state, contracts, boundaries, deployment units, recovery behavior, or durable operational obligations.

Do not start implementation or produce a recommended design until the readiness and complexity gates have been applied. Re-run the decision workflow when verified scope, invariants, ownership, or authority changes materially; do not keep patching a previously selected architecture around a new context.

## Principle

Choose the smallest complete architecture that satisfies verified requirements and protects explicit invariants across development, deployment, operation, support, recovery, upgrade, and removal.

Treat the first proposal as a candidate with positive complexity bias, not as the baseline. Optimize for useful outcome, lifecycle cost, readability, reversibility, and evidence—not theoretical completeness or local convenience.

Scale the rigor. For an obvious, reversible, single-owner change with no new state or contract, keep the analysis to a few sentences. Use the full workflow when a decision adds state, a deployment unit, a durable dependency, a cross-owner contract, difficult rollback, or a high-consequence invariant.

## Readiness gate

Before selecting a candidate, label material context as **verified fact**, **user decision**, **assumption**, or **unknown**.

If an unresolved item could materially change system boundaries, data safety, ownership, technology choice, lifecycle ranking, recovery, or acceptance criteria:

- set readiness to `blocked`;
- do not recommend, rank, default to, or conditionally select a candidate;
- state the decision hinge as the guarantee, boundary, ownership rule, or acceptance criterion it determines;
- give one next research action or user question.

Research discoverable facts before asking the user. Ask only for decisions, preferences, inaccessible facts, or boundary conditions. A “provisional,” “conservative,” or “reversible” choice does not bypass a material unknown.
